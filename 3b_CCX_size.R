...
desc <- merge(merge(merge(merge(merge(mu,unique(select(rtn,companyid, name))),md),sd),mn),mx)
rm(mu,md,sd,mn,mx)
...
write.csv2(ccx,"3b_CCX[size].csv",row.names = F,na = "")
rm(ccx)
write.csv2(fin,"3b_finCCXdata[size].csv",row.names = F,na = "")
rm(fin)
write.csv2(rtn,"3b_nonfinCCXdata[size].csv",row.names = F,na = "")
rm(rtn)
...
write.csv2(indicator,"3a_fin_indicator[size].csv",row.names = F,na = "")
rm(indicator)
...
write.csv2(indicator_rtn,"3a_nonfin_indicator[size].csv",row.names = F,na = "")
rm(indicator_rtn)
...
write.csv2(thresh,"3a_fin_threshold[size].csv",row.names = F,na = "")
rm(thresh)
...
write.csv2(thresh_rtn,"3a_nonfin_threshold[size].csv",row.names = F,na = "")
rm(thresh_rtn)
